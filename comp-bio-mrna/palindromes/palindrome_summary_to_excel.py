...
import pfind #make sure the custom python script pfind and fasta is in the same folder/directory as the current script
...
def main():
    ...
    fasta_name = sys.argv[1].split('\\')[-1] #get the file name from a windows absolute address
    
    if count_summary != 0:
        os.makedirs('csvcounts\\', exist_ok=True)
        filecsv = open(("csvcounts\\counts-"+fasta_name+"-"+sys.argv[2]+"-"+sys.argv[3]+"-"+sys.argv[4]+"-"+sys.argv[5]+".csv"), 'w')
    ...
    if palind_specifics == 1: #expSSformat, all transcripts in one file
        os.makedirs('output-forExpSS\\', exist_ok=True)
        file1 = open(("output-forExpSS\\palind-expSSform-"+fasta_name+"-"+sys.argv[2]+"-"+sys.argv[3]+"-"+sys.argv[4]+"-"+sys.argv[5]+".dat"), 'w')
    elif palind_specifics == 2:
        os.makedirs('new-output-by-palindrome\\', exist_ok=True)
        file2 = open(("new-output-by-palindrome\\"+fasta_name+"-"+sys.argv[2]+"-"+sys.argv[3]+"-"+sys.argv[4]+"-"+sys.argv[5]+".csv"), 'w')
    ...
    for ident, sequence in reader1:
        ...
        result = pfind.find_palindrome(sequence, min_length, max_length, gc_thresh, mismatch_thresh) #int, dict
